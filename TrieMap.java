...
public class TrieMap<V> extends AbstractTrieMap<V> {
    ...
    public V put(CharSequence key, V value) {
        ...
        char currChar;
        Node<V> node = root;
        for (int charIndex = 0; charIndex < key.length() - 1; charIndex++) {
            currChar = key.charAt(charIndex);
            if (!node.hasChild(currChar)) {
                node.setChild(currChar, new Node(null));
            }
            node = node.getChild(currChar);
        }
        currChar = key.charAt(key.length() - 1);
        if (!node.hasChild(currChar)) {
            size++;
            node.setChild(currChar, new Node(value));
            return null;
        }
        if (!node.getChild(currChar).hasValue()) {
            size++;
        }
        V oldValue = node.getChild(currChar).getValue();
        node.getChild(currChar).setValue(value);
        return oldValue;
    }
    ...
    public V get(CharSequence key) {
        ...
        return node.getChild(currChar).getValue();
    }
    ...
    private boolean recValue(Node<V> node, Object value) {
        ...
        Node<V>[] childrenArr = node.getChildren();

        for (Node<V> vNode : childrenArr) {
            if (vNode != null && !isFound) {
                isFound = recValue(vNode, value);
            }
        }
        return isFound;
    }
    ...
    public boolean containsValue(Object value) {
        ...
        return recValue(root, value);
    }

    private boolean otherChildrenExist(Node<V> node) {
        Node<V>[] childrenArr = node.getChildren();
        int counter = 0;
        for (Node<V> vNode : childrenArr) {
            if (vNode != null) {
    ...
    public V remove(CharSequence key) {
        ...
        Node<V>[] indexCharNodeArr = new Node[key.length()];
        char currChar;
        Node<V> node = root;
        for (int charIndex = 0; charIndex < key.length(); charIndex++) {
            currChar = key.charAt(charIndex);
            indexCharNodeArr[charIndex] = node;
            //0-key.length()-2, key.length()-1, last array is last char's parent
            node = node.getChild(currChar);
        }
        ans = node.getValue();
        node.setValue(null);
        ...
        int decCounter = key.length() - 1;
        while (decCounter >= 0) {
            currChar = key.charAt(decCounter);
            if (indexCharNodeArr[decCounter].hasValue()) {
                indexCharNodeArr[decCounter].initChildren();
                return ans;
                //find parent with indexCharNodeArr
                //and see if they have either another child or a value, then stop removing
            }
            if (otherChildrenExist(indexCharNodeArr[decCounter])) {
                indexCharNodeArr[decCounter].setChild(currChar, null);
                return ans;
            }
            indexCharNodeArr[decCounter].initChildren();
            decCounter--;
    ...
    static class Node<V> {
        ...
        public boolean hasChild(char c) {
            ...
            return children[convertToIndex(c)] != null;
        }
