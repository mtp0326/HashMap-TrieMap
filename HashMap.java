...
public class HashMap<K, V> extends BaseAbstractMap<K, V> {
    ...
    private Entry<K, V> findEntry(Entry<K, V> currEntry, Object key) {
        if (currEntry == null) {
            return null;
        }
        if ((currEntry.getKey() == null && key == null)
                || (currEntry.getKey() != null && currEntry.getKey().equals(key))) {
            return currEntry;
        }
    ...
    public V get(Object key) {
        ...
        if (key == null) {
            ...
            Entry<K, V> currEntry = table[0];
            Entry<K, V> ans = findEntry(currEntry, key);
            if (ans == null) {
                return null;
            }
            return ans.getValue();
        }
        if (hash(key.hashCode(), table.length) >= table.length) {
            return null;
        }
        Entry<K, V> currEntry = table[hash(key.hashCode(), table.length)];
        Entry<K, V> ans = findEntry(currEntry, key);
        if (ans == null) {
            return null;
        }
    ...
    public boolean containsKey(Object key) {
        ...
        int hashKey = 0;
        if (key != null) {
            hashKey = hash(key.hashCode(), table.length);
        }
        if (hashKey >= table.length) {
            return false;
        }
        return findEntry(table[hashKey], key) != null;
    }
    ...
    public V put(K key, V value) {
        ...
        if (containsKey(key)) {
            ...
            return findEntry(table[hashKey], key).setValue(value);
        }
        size++;
        if (size() >= threshold) {
        ...
        Entry<K, V> currEntry = table[hashKey];
        if (currEntry == null) {
            table[hashKey] = new Entry<>(key, value, null);
            return null;
        }
        while (currEntry.next != null) {
            currEntry = currEntry.next;
        }
        currEntry.next = new Entry<>(key, value, null);
        return null;
    }
    ...
    void resize(int newCapacity) {
        ...
        Iterator<Map.Entry<K, V>> iterEntry = entryIterator();
        Entry<K, V>[] newEntryTable = new Entry[newCapacity];
        this.table = newEntryTable;
        threshold = (int) (newCapacity * loadFactor);
        while (iterEntry.hasNext()) {
            Entry<K, V> newEntry = (Entry<K, V>) iterEntry.next();
            int hashKey = hash(newEntry.getKey().hashCode(), newCapacity);
    ...
    public V remove(Object key) {
        ...
        Entry<K, V> currEntry = table[hashKey];
        if (currEntry != null || containsKey(key)) {
            size--;
            if ((key == null && currEntry.getKey() == null)
                    || (currEntry.getKey() != null && currEntry.getKey().equals(key))) {
                V ans = currEntry.getValue();
                if (currEntry.next == null) {
                    table[hashKey] = null;
                } else {
                    table[hashKey] = currEntry.next;
                }
                return ans;
            }
    ...
    public boolean containsValue(Object value) {
        ...
        while (iterEntry.hasNext()) {
            Entry<K, V> newEntry = (Entry<K, V>) iterEntry.next();
            if ((value == null && newEntry.getValue() == null)
                    || (newEntry.getValue() != null && newEntry.getValue().equals(value))) {
                return true;
            }
        }
        return false;
    ...
    protected Iterator<Map.Entry<K, V>> entryIterator() {
        ...
        return new IteratorOverride(table);
    }

    private class IteratorOverride implements Iterator<Map.Entry<K, V>> {
        private Entry<K, V> currEntry;
        int index = 0;
        Entry<K, V>[] hmTable;

        public IteratorOverride(Entry<K, V>[] table) {
            this.hmTable = table;

            while (index < hmTable.length && hmTable[index] == null) {
                index++;
            }
            //completely empty table:::
            if (index < hmTable.length) {
                currEntry = hmTable[index];
            } else {
                currEntry = null;
            }
        }
        ...
        public Entry<K, V> next() {
            if (hasNext()) {
                Entry<K, V> oldEntry = new Entry<>(currEntry.getKey(), currEntry.getValue(), null);
                if (currEntry.next != null) {
                    currEntry = currEntry.next;
                } else {
                    index++;
